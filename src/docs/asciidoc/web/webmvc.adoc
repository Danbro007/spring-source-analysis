...
Spring MVC, as many other web frameworks, is designed around the front com.danbro.springmvc.controller
...
| Help the `DispatcherServlet` to invoke a handler mapped to a request, regardless of
  how the handler is actually invoked. For example, invoking an annotated com.danbro.springmvc.controller
...
* The `WebApplicationContext` is searched for and bound in the request as an attribute
  that the com.danbro.springmvc.controller and other elements in the process can use. It is bound by default
...
| Whether to throw a `NoHandlerFoundException` when no handler was found for a request.
  ...
  `@ExceptionHandler` com.danbro.springmvc.controller method) and handled as any others.
...
declare it as an <<mvc-ann-com.danbro.springmvc.controller-advice>> bean or configure it directly on
...
<<mvc-config-view-com.danbro.springmvc.controller, View Controllers>> which are useful for HTML template
rendering without com.danbro.springmvc.controller logic.
...
The net effect is the same as if the com.danbro.springmvc.controller had returned a `RedirectView`, but now
the com.danbro.springmvc.controller itself can operate in terms of logical view names. A logical view
...
Note that, if a com.danbro.springmvc.controller method is annotated with the `@ResponseStatus`, the annotation
...
for each request. Other strategies at the com.danbro.springmvc.controller level, described earlier, can avoid
...
[[mvc-com.danbro.springmvc.controller]]
== Annotated Controllers
[.small]#<<web-reactive.adoc#webflux-com.danbro.springmvc.controller, Same as in Spring WebFlux>>#
...
The following example shows a com.danbro.springmvc.controller defined by annotations:
...
[[mvc-ann-com.danbro.springmvc.controller]]
=== Declaration
[.small]#<<web-reactive.adoc#webflux-ann-com.danbro.springmvc.controller, Same as in Spring WebFlux>>#

You can define com.danbro.springmvc.controller beans by using a standard Spring bean definition in the
...
itself meta-annotated with `@Controller` and `@ResponseBody` to indicate a com.danbro.springmvc.controller whose
...
In some cases, you may need to decorate a com.danbro.springmvc.controller with an AOP proxy at runtime.
One example is if you choose to have `@Transactional` annotations directly on the
com.danbro.springmvc.controller. When this is the case, for controllers specifically, we recommend
using class-based proxying. This is typically the default choice with controllers.
However, if a com.danbro.springmvc.controller must implement an interface that is not a Spring Context
...
arguably, most com.danbro.springmvc.controller methods should be mapped to a specific HTTP method versus
...
com.danbro.springmvc.controller mapped to `/person` is also implicitly mapped to `/person.{asterisk}`.
...
content types that a com.danbro.springmvc.controller method produces, as the following example shows:
...
com.danbro.springmvc.controller methods should be mapped to a specific HTTP method versus using `@RequestMapping`,
...
supported com.danbro.springmvc.controller method arguments and return values.
...
The next table describes the supported com.danbro.springmvc.controller method arguments. Reactive types are not supported
for any arguments.
...
The next table describes the supported com.danbro.springmvc.controller method return values. Reactive types are
...
| A method with a `void` return type (or `null` return value) is considered to have fully
  ...
  an `@ResponseStatus` annotation. The same is also true if the com.danbro.springmvc.controller has made a positive
...
Some annotated com.danbro.springmvc.controller method arguments that represent `String`-based request input (such as
...
If a URL is expected to contain matrix variables, the request mapping for a com.danbro.springmvc.controller
...
query parameters or form data) to a method argument in a com.danbro.springmvc.controller.
...
in a com.danbro.springmvc.controller.
...
for such errors in the com.danbro.springmvc.controller method, you can add a `BindingResult` argument immediately next
...
cases, you can inject the `Model` into the com.danbro.springmvc.controller and access it directly or,
...
specific com.danbro.springmvc.controller. This typically lists the names of model attributes or types of
...
until another com.danbro.springmvc.controller method uses a `SessionStatus` method argument to clear the
...
(that is, outside the com.danbro.springmvc.controller -- for example, by a filter) and may or may not be present,
...
`javax.servlet.http.HttpSession` into the com.danbro.springmvc.controller method.

For temporary storage of model attributes in the session as part of a com.danbro.springmvc.controller
...
`Model` should never be used if a com.danbro.springmvc.controller method redirects. Instead, the com.danbro.springmvc.controller
...
`Model` of the com.danbro.springmvc.controller that serves the target URL.
...
within the com.danbro.springmvc.controller through an `Errors` or `BindingResult` argument,
as the following example shows:
...
all com.danbro.springmvc.controller methods. This is the effect of `@RestController`, which is nothing more
...
`@ResponseBody` or `ResponseEntity` com.danbro.springmvc.controller methods, you can use Jackson's
...
com.danbro.springmvc.controller method. If you need to activate multiple views, you can use a composite interface.
...
A com.danbro.springmvc.controller can have any number of `@ModelAttribute` methods. All such methods are
invoked before `@RequestMapping` methods in the same com.danbro.springmvc.controller. A `@ModelAttribute`
method can also be shared across controllers through `@ControllerAdvice`. See the section on
<<mvc-ann-com.danbro.springmvc.controller-advice>> for more details.
...
headers, cookies, and others) to the target type of com.danbro.springmvc.controller method arguments.
* Format model object values as `String` values when rendering HTML forms.

`@InitBinder` methods can register com.danbro.springmvc.controller-specific `java.bean.PropertyEditor` or
...
com.danbro.springmvc.controller-specific `Formatter` implementations, as the following example shows:
...
=== Exceptions
[.small]#<<web-reactive.adoc#webflux-ann-com.danbro.springmvc.controller-exceptions, Same as in Spring WebFlux>>#

`@Controller` and <<mvc-ann-com.danbro.springmvc.controller-advice, @ControllerAdvice>> classes can have
`@ExceptionHandler` methods to handle exceptions from com.danbro.springmvc.controller methods, as the following example shows:
...
com.danbro.springmvc.controller or `@ControllerAdvice` class. This means a cause match on a higher-priority
...
| `HandlerMethod`
| For access to the com.danbro.springmvc.controller method that raised the exception.
...
| A method with a `void` return type (or `null` return value) is considered to have fully
  handled the response if it also has a `ServletResponse` an `OutputStream` argument, or
  a `@ResponseStatus` annotation. The same is also true if the com.danbro.springmvc.controller has made a positive
  `ETag` or `lastModified` timestamp check (see <<mvc-caching-etag-lastmodified>> for details).
...
[[mvc-ann-com.danbro.springmvc.controller-advice]]
=== Controller Advice
[.small]#<<web-reactive.adoc#webflux-ann-com.danbro.springmvc.controller-advice, Same as in Spring WebFlux>>#
...
Spring MVC provides a mechanism to prepare links to com.danbro.springmvc.controller methods. For example,
the following MVC com.danbro.springmvc.controller allows for link creation:
...
akin to mock testing through proxies to avoid referring to the com.danbro.springmvc.controller method by name, as the following example shows
...
Given the preceding com.danbro.springmvc.controller, you can prepare a link from a JSP, as follows:
...
return values in com.danbro.springmvc.controller methods and provide basic support for a single asynchronous
return value.
...
in the Servlet container, com.danbro.springmvc.controller methods can wrap any supported com.danbro.springmvc.controller method
...
The com.danbro.springmvc.controller can produce the return value asynchronously, from a different thread -- for
...
A com.danbro.springmvc.controller can wrap any supported return value with `java.util.concurrent.Callable`,
as the following example shows:
...
* The com.danbro.springmvc.controller returns a `DeferredResult` and saves it in some in-memory
...
* The com.danbro.springmvc.controller returns a `Callable`.
...
com.danbro.springmvc.controller method returned the given value or as if it produced the given exception.
...
asynchronous support is built around that mechanism. When a com.danbro.springmvc.controller returns a `DeferredResult`,
...
com.danbro.springmvc.controller is mapped again but, rather than invoking it, the `DeferredResult` value is used
(as if the com.danbro.springmvc.controller returned it) to resume processing.
...
asynchronous and <<mvc-ann-async-reactive-types>> as return values in com.danbro.springmvc.controller methods.
...
types in com.danbro.springmvc.controller method arguments (for example, `@RequestBody`, `@RequestPart`, and others),
...
stream from a com.danbro.springmvc.controller, return `SseEmitter`, as the following example shows:
...
Spring MVC supports use of reactive client libraries in a com.danbro.springmvc.controller (also read
...
reactive types from the com.danbro.springmvc.controller method.
...
com.danbro.springmvc.controller methods. We highly recommended configuring this property if you
stream with reactive types or have com.danbro.springmvc.controller methods that return `Callable`, since
...
against conditional request headers. A com.danbro.springmvc.controller can add an `ETag` header and `Cache-Control`
...
You can also make the check against conditional request headers in the com.danbro.springmvc.controller,
as the following example shows:
...
`Validated` on com.danbro.springmvc.controller method arguments.
...
[[mvc-config-view-com.danbro.springmvc.controller]]
=== View Controllers

This is a shortcut for defining a `ParameterizableViewController` that immediately
forwards to a view when invoked. You can use it in static cases when there is no Java com.danbro.springmvc.controller
...
using the `<mvc:view-com.danbro.springmvc.controller>` element:

[source,xml,indent=0,subs="verbatim,quotes"]
----
	<mvc:view-com.danbro.springmvc.controller path="/" view-name="home"/>
----
