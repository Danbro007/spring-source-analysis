...
Spring MVC, as many other web frameworks, is designed around the front com.catchArg.springmvc.controller
...
| Help the `DispatcherServlet` to invoke a handler mapped to a request, regardless of
  how the handler is actually invoked. For example, invoking an annotated com.catchArg.springmvc.controller
...
* The `WebApplicationContext` is searched for and bound in the request as an attribute
  that the com.catchArg.springmvc.controller and other elements in the process can use. It is bound by default
...
| Whether to throw a `NoHandlerFoundException` when no handler was found for a request.
  ...
  `@ExceptionHandler` com.catchArg.springmvc.controller method) and handled as any others.
...
declare it as an <<mvc-ann-com.catchArg.springmvc.controller-advice>> bean or configure it directly on
...
<<mvc-config-view-com.catchArg.springmvc.controller, View Controllers>> which are useful for HTML template
rendering without com.catchArg.springmvc.controller logic.
...
The net effect is the same as if the com.catchArg.springmvc.controller had returned a `RedirectView`, but now
the com.catchArg.springmvc.controller itself can operate in terms of logical view names. A logical view
...
Note that, if a com.catchArg.springmvc.controller method is annotated with the `@ResponseStatus`, the annotation
...
for each request. Other strategies at the com.catchArg.springmvc.controller level, described earlier, can avoid
...
[[mvc-com.catchArg.springmvc.controller]]
== Annotated Controllers
[.small]#<<web-reactive.adoc#webflux-com.catchArg.springmvc.controller, Same as in Spring WebFlux>>#
...
The following example shows a com.catchArg.springmvc.controller defined by annotations:
...
[[mvc-ann-com.catchArg.springmvc.controller]]
=== Declaration
[.small]#<<web-reactive.adoc#webflux-ann-com.catchArg.springmvc.controller, Same as in Spring WebFlux>>#

You can define com.catchArg.springmvc.controller beans by using a standard Spring bean definition in the
...
itself meta-annotated with `@Controller` and `@ResponseBody` to indicate a com.catchArg.springmvc.controller whose
...
In some cases, you may need to decorate a com.catchArg.springmvc.controller with an AOP proxy at runtime.
One example is if you choose to have `@Transactional` annotations directly on the
com.catchArg.springmvc.controller. When this is the case, for controllers specifically, we recommend
using class-based proxying. This is typically the default choice with controllers.
However, if a com.catchArg.springmvc.controller must implement an interface that is not a Spring Context
...
arguably, most com.catchArg.springmvc.controller methods should be mapped to a specific HTTP method versus
...
com.catchArg.springmvc.controller mapped to `/person` is also implicitly mapped to `/person.{asterisk}`.
...
content types that a com.catchArg.springmvc.controller method produces, as the following example shows:
...
com.catchArg.springmvc.controller methods should be mapped to a specific HTTP method versus using `@RequestMapping`,
...
supported com.catchArg.springmvc.controller method arguments and return values.
...
The next table describes the supported com.catchArg.springmvc.controller method arguments. Reactive types are not supported
for any arguments.
...
The next table describes the supported com.catchArg.springmvc.controller method return values. Reactive types are
...
| A method with a `void` return type (or `null` return value) is considered to have fully
  ...
  an `@ResponseStatus` annotation. The same is also true if the com.catchArg.springmvc.controller has made a positive
...
Some annotated com.catchArg.springmvc.controller method arguments that represent `String`-based request input (such as
...
If a URL is expected to contain matrix variables, the request mapping for a com.catchArg.springmvc.controller
...
query parameters or form data) to a method argument in a com.catchArg.springmvc.controller.
...
com.catchArg.springmvc.controller.
...
in a com.catchArg.springmvc.controller.
...
for such errors in the com.catchArg.springmvc.controller method, you can add a `BindingResult` argument immediately next
...
cases, you can inject the `Model` into the com.catchArg.springmvc.controller and access it directly or,
...
specific com.catchArg.springmvc.controller. This typically lists the names of model attributes or types of
...
until another com.catchArg.springmvc.controller method uses a `SessionStatus` method argument to clear the
...
(that is, outside the com.catchArg.springmvc.controller -- for example, by a filter) and may or may not be present,
...
`javax.servlet.http.HttpSession` into the com.catchArg.springmvc.controller method.

For temporary storage of model attributes in the session as part of a com.catchArg.springmvc.controller
...
`Model` should never be used if a com.catchArg.springmvc.controller method redirects. Instead, the com.catchArg.springmvc.controller
...
`Model` of the com.catchArg.springmvc.controller that serves the target URL.
...
within the com.catchArg.springmvc.controller through an `Errors` or `BindingResult` argument,
as the following example shows:
...
all com.catchArg.springmvc.controller methods. This is the effect of `@RestController`, which is nothing more
...
`@ResponseBody` or `ResponseEntity` com.catchArg.springmvc.controller methods, you can use Jackson's
...
com.catchArg.springmvc.controller method. If you need to activate multiple views, you can use a composite interface.
...
A com.catchArg.springmvc.controller can have any number of `@ModelAttribute` methods. All such methods are
invoked before `@RequestMapping` methods in the same com.catchArg.springmvc.controller. A `@ModelAttribute`
method can also be shared across controllers through `@ControllerAdvice`. See the section on
<<mvc-ann-com.catchArg.springmvc.controller-advice>> for more details.
...
headers, cookies, and others) to the target type of com.catchArg.springmvc.controller method arguments.
* Format model object values as `String` values when rendering HTML forms.

`@InitBinder` methods can register com.catchArg.springmvc.controller-specific `java.bean.PropertyEditor` or
...
com.catchArg.springmvc.controller-specific `Formatter` implementations, as the following example shows:
...
=== Exceptions
[.small]#<<web-reactive.adoc#webflux-ann-com.catchArg.springmvc.controller-exceptions, Same as in Spring WebFlux>>#

`@Controller` and <<mvc-ann-com.catchArg.springmvc.controller-advice, @ControllerAdvice>> classes can have
`@ExceptionHandler` methods to handle exceptions from com.catchArg.springmvc.controller methods, as the following example shows:
...
com.catchArg.springmvc.controller or `@ControllerAdvice` class. This means a cause match on a higher-priority
...
| `HandlerMethod`
| For access to the com.catchArg.springmvc.controller method that raised the exception.
...
| A method with a `void` return type (or `null` return value) is considered to have fully
  handled the response if it also has a `ServletResponse` an `OutputStream` argument, or
  a `@ResponseStatus` annotation. The same is also true if the com.catchArg.springmvc.controller has made a positive
  `ETag` or `lastModified` timestamp check (see <<mvc-caching-etag-lastmodified>> for details).
...
[[mvc-ann-com.catchArg.springmvc.controller-advice]]
=== Controller Advice
[.small]#<<web-reactive.adoc#webflux-ann-com.catchArg.springmvc.controller-advice, Same as in Spring WebFlux>>#
...
Spring MVC provides a mechanism to prepare links to com.catchArg.springmvc.controller methods. For example,
the following MVC com.catchArg.springmvc.controller allows for link creation:
...
akin to mock testing through proxies to avoid referring to the com.catchArg.springmvc.controller method by name, as the following example shows
...
Given the preceding com.catchArg.springmvc.controller, you can prepare a link from a JSP, as follows:
...
return values in com.catchArg.springmvc.controller methods and provide basic support for a single asynchronous
return value.
...
in the Servlet container, com.catchArg.springmvc.controller methods can wrap any supported com.catchArg.springmvc.controller method
...
The com.catchArg.springmvc.controller can produce the return value asynchronously, from a different thread -- for
...
A com.catchArg.springmvc.controller can wrap any supported return value with `java.util.concurrent.Callable`,
as the following example shows:
...
* The com.catchArg.springmvc.controller returns a `DeferredResult` and saves it in some in-memory
...
* The com.catchArg.springmvc.controller returns a `Callable`.
...
com.catchArg.springmvc.controller method returned the given value or as if it produced the given exception.
...
asynchronous support is built around that mechanism. When a com.catchArg.springmvc.controller returns a `DeferredResult`,
...
com.catchArg.springmvc.controller is mapped again but, rather than invoking it, the `DeferredResult` value is used
(as if the com.catchArg.springmvc.controller returned it) to resume processing.
...
asynchronous and <<mvc-ann-async-reactive-types>> as return values in com.catchArg.springmvc.controller methods.
...
types in com.catchArg.springmvc.controller method arguments (for example, `@RequestBody`, `@RequestPart`, and others),
...
stream from a com.catchArg.springmvc.controller, return `SseEmitter`, as the following example shows:
...
Spring MVC supports use of reactive client libraries in a com.catchArg.springmvc.controller (also read
...
reactive types from the com.catchArg.springmvc.controller method.
...
com.catchArg.springmvc.controller methods. We highly recommended configuring this property if you
stream with reactive types or have com.catchArg.springmvc.controller methods that return `Callable`, since
...
against conditional request headers. A com.catchArg.springmvc.controller can add an `ETag` header and `Cache-Control`
...
You can also make the check against conditional request headers in the com.catchArg.springmvc.controller,
as the following example shows:
...
`Validated` on com.catchArg.springmvc.controller method arguments.
...
[[mvc-config-view-com.catchArg.springmvc.controller]]
=== View Controllers

This is a shortcut for defining a `ParameterizableViewController` that immediately
forwards to a view when invoked. You can use it in static cases when there is no Java com.catchArg.springmvc.controller
...
using the `<mvc:view-com.catchArg.springmvc.controller>` element:

[source,xml,indent=0,subs="verbatim,quotes"]
----
	<mvc:view-com.catchArg.springmvc.controller path="/" view-name="home"/>
----
