...
* <<webflux-com.danbro.springmvc.controller>>: Consistent with Spring MVC and based on the same annotations
...
directly from Spring MVC com.danbro.springmvc.controller methods. The greater the latency per call or the
...
`HttpMessageReader<Part>` instead. For example, in an annotated com.danbro.springmvc.controller, use of
...
Spring WebFlux, similarly to Spring MVC, is designed around the front com.danbro.springmvc.controller pattern,
...
See also <<webflux-ann-com.danbro.springmvc.controller-exceptions>> in the "`Annotated Controller`" section or
...
The net effect is the same as if the com.danbro.springmvc.controller had returned a `RedirectView` or
`Rendering.redirectTo("abc").build()`, but now the com.danbro.springmvc.controller itself can
...
[[webflux-com.danbro.springmvc.controller]]
== Annotated Controllers
[.small]#<<web.adoc#mvc-com.danbro.springmvc.controller, Same as in Spring MVC>>#
...
[[webflux-ann-com.danbro.springmvc.controller]]
=== `@Controller`
[.small]#<<web.adoc#mvc-ann-com.danbro.springmvc.controller, Same as in Spring MVC>>#

You can define com.danbro.springmvc.controller beans by using a standard Spring bean definition.
...
itself meta-annotated with `@Controller` and `@ResponseBody`, indicating a com.danbro.springmvc.controller whose
...
because, arguably, most com.danbro.springmvc.controller methods should be mapped to a specific HTTP method versus
...
The following table shows the supported com.danbro.springmvc.controller method arguments.
...
The following table shows the supported com.danbro.springmvc.controller method return values. Note that reactive
...
| A method with a `void`, possibly asynchronous (for example, `Mono<Void>`), return type (or a `null` return
  ...
  if the com.danbro.springmvc.controller has made a positive ETag or `lastModified` timestamp check.
...
Some annotated com.danbro.springmvc.controller method arguments that represent String-based request input (for example,
...
com.danbro.springmvc.controller method, you need to add a URI variable to the path segment where matrix
...
com.danbro.springmvc.controller. The following code snippet shows the usage:
...
to check for such errors in the com.danbro.springmvc.controller method, you can add a `BindingResult` argument
...
another com.danbro.springmvc.controller method uses a `SessionStatus` method argument to clear the storage,
as the following example shows:
...
`WebSession` into the com.danbro.springmvc.controller method.
...
content. The best way to handle a file upload form (for example, from a browser) in a com.danbro.springmvc.controller
...
within the com.danbro.springmvc.controller through an `Errors` or `BindingResult` argument, as the following example shows:
...
By default, validation errors cause a `WebExchangeBindException`, which is turned
into a 400 (`BAD_REQUEST`) response. Alternatively, you can handle validation errors locally
within the com.danbro.springmvc.controller through an `Errors` or a `BindingResult` argument. The following
...
com.danbro.springmvc.controller method. Use a composite interface if you need to activate multiple views.
...
<<webflux-ann-com.danbro.springmvc.controller-advice>> for more details.
...
com.danbro.springmvc.controller-specific `Formatter` instances, as the following example shows:
...
[[webflux-ann-com.danbro.springmvc.controller-exceptions]]
=== Managing Exceptions
...
`@ExceptionHandler` methods to handle exceptions from com.danbro.springmvc.controller methods. The following
...
[[webflux-ann-com.danbro.springmvc.controller-advice]]
=== Controller Advice
[.small]#<<web.adoc#mvc-ann-com.danbro.springmvc.controller-advice, Same as in Spring MVC>>#
...
against conditional request headers. A com.danbro.springmvc.controller can add an `ETag` and `Cache-Control`
