...
[[webflux-cors-com.catchArg.springmvc.controller]]
== `@CrossOrigin`
[.small]#<<web.adoc#mvc-cors-com.catchArg.springmvc.controller, Same as in Spring MVC>>#

The {api-spring-framework}/web/bind/annotation/CrossOrigin.html[`@CrossOrigin`]
annotation enables cross-origin requests on annotated com.catchArg.springmvc.controller methods, as the
