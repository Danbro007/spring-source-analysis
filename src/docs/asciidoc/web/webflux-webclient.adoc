...
[NOTE]
====
With `Flux` or `Mono`, you should never have to block in a Spring MVC or Spring WebFlux com.danbro.springmvc.controller.
Simply return the resulting reactive type from the com.danbro.springmvc.controller method. The same principle apply to
Kotlin Coroutines and Spring WebFlux, just use suspending function or return `Flow` in your
com.danbro.springmvc.controller method .
====
