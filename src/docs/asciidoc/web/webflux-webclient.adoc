...
[NOTE]
====
With `Flux` or `Mono`, you should never have to block in a Spring MVC or Spring WebFlux com.catchArg.springmvc.controller.
Simply return the resulting reactive type from the com.catchArg.springmvc.controller method. The same principle apply to
Kotlin Coroutines and Spring WebFlux, just use suspending function or return `Flow` in your
com.catchArg.springmvc.controller method .
====
